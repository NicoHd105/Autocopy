...
if (!SOURCE_FILE_PATH && !SOURCE_FOLDER_PATH) throw new TypeError(chalk.red.bold('Neither a source file or a source folder have been provided!'));
if (SOURCE_FILE_PATH && !fs.existsSync(SOURCE_FILE_PATH)) throw new TypeError(chalk.red.bold(`The source file path ${chalk.yellow(SOURCE_FILE_PATH)} is not a valid path!`));
if (SOURCE_FOLDER_PATH && !fs.existsSync(SOURCE_FOLDER_PATH)) throw new TypeError(chalk.red.bold(`The source folder path ${chalk.yellow(SOURCE_FOLDER_PATH)} is not a valid path!`));
...
if (!DESTINATION_PATH) {
	...
	throw new TypeError(chalk.red.bold(`The destiantion path ${chalk.yellow(DESTINATION_PATH.replace(/[^/]*$/, ''))} is not a valid path!`));
}
if (!INTERVALTIME) {
	...
	throw new TypeError(
		chalk.red.bold(
			`The interval time is not a valid number!\nYou can use ${chalk.yellow('d')} for days, ${chalk.yellow('h')} for hours, ${chalk.yellow('m')} for minutes, ${chalk.yellow(
				's'
			)} for seconds and ${chalk.yellow('ms')} for milliseconds!`
		)
	);
}
...
setInterval(() => {
	...
	if (SOURCE_FILE_PATH && !fs.existsSync(SOURCE_FILE_PATH)) throw new TypeError(chalk.red.bold(`The source file path ${chalk.yellow(SOURCE_FILE_PATH)} is not a valid path anymore!`));
	if (SOURCE_FOLDER_PATH && !fs.existsSync(SOURCE_FOLDER_PATH))
		throw new TypeError(chalk.red.bold(`The source folder path ${chalk.yellow(SOURCE_FOLDER_PATH)} is not a valid path anymore!`));
	if (!fs.existsSync(DESTINATION_PATH.replace(/[^/]*$/, '')))
		throw new TypeError(chalk.red.bold(`The destiantion path ${chalk.yellow(DESTINATION_PATH.replace(/[^/]*$/, ''))} is not a valid path anymore!`));
