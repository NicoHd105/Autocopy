	...
	console.log(
		chalk.green.bold(
			`The ${SOURCE_FILE_PATH ? `${chalk.yellow.underline(SOURCE_FILE_PATH.split('/').pop())} file` : ''} ${
				SOURCE_FOLDER_PATH ? `${SOURCE_FILE_PATH ? 'and the ' : ''}${chalk.yellow.underline(SOURCE_FOLDER_PATH.split('/').pop())} folder ${SOURCE_FILE_PATH ? 'were' : 'was'}` : 'was'
			} successfully copied to the ${chalk.yellow.underline(
				`${DESTINATION_PATH}/${DATA_FOLDER_NAME}${DATA_FOLDER_SEPERATOR}${i}`
			)} directory! | #${i} | ${`${date.getFullYear()}-${dateTimePad(date.getMonth() + 1, 2)}-${dateTimePad(date.getDate(), 2)} ${dateTimePad(date.getHours(), 2)}:${dateTimePad(
				date.getMinutes(),
				2
